...
public:
    ...
    NonStandard_Timeline(int TOT, int TOF, int MTOF, std::vector<fireRange> firingRanges, int fireFreq);
    ...
    */
    ...
    /**
     * Adds a new marking round into the timelist.
     *
     * @param mType the marking type of the marking round
     * 
     * @param markingTOT the time the marking round should hit the target in seconds
     */
    void addMarkingRound(markingType mType, int markingTOT);

    /**
     * Adds a new firing range into the timelist
     *
     */
    void addFireRange(fireRange newFR);
    /**
     * Getter method for Marking Time of Flight.
     *
     * @param newFR the new firing range to be added to the timelist
     */
private:
    /**
     * Fills the `timeList` table with Sexagesimal times that represent when each shell should be fired.
