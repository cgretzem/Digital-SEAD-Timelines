...
void MainWindow::clearErrorStatus(){
...
void MainWindow::on_fileNameButton_clicked()
{
    ...
    //check is file already exists
        ...
        if(file.is_open()){
            ...
            errorOccured = true;
            ui->errorWindow->show();
            ui->errorWindow->setText(ui->errorWindow->toPlainText() + "ERROR: File Name Already Exist on Desktop");
        }
    }
    ...
    if(!errorOccured){
        ...
        myfile.open(path); //creates a file
...
bool MainWindow::CalculateTimelineErrors(int timelineType, string *inputs){
    ...
    if(!isInt(CAS_TOT)){//is CAS_TOT an integer?
        errorOccurred = true;
        ui->errorWindow->setText(ui->errorWindow->toPlainText() + "ERROR: CAS TOT is not a whole number\n");
    }
    if(!invalidChar(CAS_TOT)){//does CAS_TOT contain an invalid character?
        errorOccurred = true;
        ui->errorWindow->setText(ui->errorWindow->toPlainText() + "ERROR: CAS TOT contains invalid characters\n");
    }
    if(!check4neg(CAS_TOT)){//is CAS_TOT a negative number?
        errorOccurred = true;
        ui->errorWindow->setText(ui->errorWindow->toPlainText() + "ERROR: CAS TOT must be a postive number\n");
    }
...
void MainWindow::on_CalculateTimelineButton_clicked()
{
    ...
    if(!errorOccured){
       ...
       clearErrorStatus();
        ...
        if(ui->fireTypeSelector->currentIndex() == 0 || ui->fireTypeSelector->currentIndex() == 1){
        ...
        //handle non-standard timelines
            ...
            if(mt != NEGATIVE){ for(int num : frVecMark){ timeline.addMarkingRound(mt, num); } }
            ...
            if(mt != NEGATIVE){
                ...
                for(int i : frVecMark){
                printInput[gunIndex].mAt += to_string(i);
                printInput[gunIndex].mAt += ", ";
                }
